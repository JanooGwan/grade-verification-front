...
export default function StudentDataEditor({ transcript, onDeleted }: { transcript: StudentTranscript; onDeleted: () => void }) {
  ...
  const deleteCourseMutation = useMutation({ mutationFn: (id: number) => deleteTranscriptCourse(transcript.studentId, id), onSuccess: refresh });
  const deleteStudentMutation = useMutation({ mutationFn: () => deleteStudent(transcript.studentId), onSuccess: async () => { await refresh(); onDeleted(); } });
  ...
  return <details className="student-data-editor">
    ...
    <form className="student-profile-form" onSubmit={(event) => { event.preventDefault(); profileMutation.mutate(); }}>
      ...
      <button>기본정보 저장</button><button className="danger-action" type="button" onClick={() => window.confirm('학생과 모든 성적·지원·검증 이력을 삭제할까요?') && deleteStudentMutation.mutate()}>학생 삭제</button>
    </form>
    <form className="course-editor-form" onSubmit={(event: FormEvent) => { event.preventDefault(); courseMutation.mutate(); }}>
      ...
      <button>{courseId ? '과목 수정' : '과목 추가'}</button>{courseId > 0 && <button className="danger-action" type="button" onClick={() => window.confirm('이 과목을 삭제할까요?') && deleteCourseMutation.mutate(courseId)}>과목 삭제</button>}
    </form>
  </details>;
}
