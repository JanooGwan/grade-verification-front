...
import { importTranscriptExcel, previewTranscriptExcel } from '@/apis/transcript';
...
export default function TranscriptImportPanel({ admissionYear }: { admissionYear: number }) {
  ...
  const preview = useMutation({ mutationFn: () => previewTranscriptExcel(admissionYear, file as File) });
  const importer = useMutation({
    mutationFn: () => importTranscriptExcel(admissionYear, mode, file as File),
    onSuccess: async () => {
  ...
  const error = preview.error ?? importer.error ?? history.error;
  return (
    <details className="transcript-import-panel">
      ...
      <form className="transcript-import-form" onSubmit={submitPreview}>
        ...
        <label>Excel 파일<input type="file" accept=".xlsx,.xls" onChange={(event) => { setFile(event.target.files?.[0] ?? null); preview.reset(); importer.reset(); }} /></label>
        ...
        <button disabled={!file || preview.isPending}>{preview.isPending ? '분석 중…' : '미리보기'}</button>
      ...
      {preview.data && <div className="import-preview">
        <div className="import-preview-summary"><span>전체 <b>{preview.data.totalRows}</b></span><span>정상 <b>{preview.data.validRows}</b></span><span className={preview.data.invalidRows ? 'has-error' : ''}>오류 <b>{preview.data.invalidRows}</b></span><button type="button" disabled={importer.isPending || (mode === 'ALL_OR_NOTHING' && preview.data.invalidRows > 0)} onClick={() => importer.mutate()}>{importer.isPending ? '저장 중…' : 'DB 저장'}</button></div>
        {preview.data.errors.length > 0 && <div className="import-errors">{preview.data.errors.slice(0, 20).map((item) => <p key={`${item.rowNumber}-${item.message}`}><b>{item.rowNumber}행</b> {item.message}</p>)}</div>}
      ...
      {importer.data && <p className="import-success">가져오기 #{importer.data.importId}: 학생 {importer.data.createdStudents}명 생성, 과목 {importer.data.createdCourses}건 생성·{importer.data.updatedCourses}건 수정</p>}
